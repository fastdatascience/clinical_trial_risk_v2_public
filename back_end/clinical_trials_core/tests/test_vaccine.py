...
class TestVaccineExtractor(unittest.TestCase):
    ...
    def test_vaccine(self):
        document = Document(pages=[Page(content="this is a vaccine trial", page_number=1)])
        d_result = d.process(document=document)
    ...
    def test_annotations_correct_text(self):
        document = Document(pages=[Page(content="this is a trial for a vaccine", page_number=1)])
        d_result = d.process(document=document)
        self.assertEqual("vaccine", d_result["annotations"][0]["text"])

    def test_annotations_correct_char_indices(self):
        document = Document(pages=[Page(content="this is an vaccine trial in the USA", page_number=1)])
        d_result = d.process(document=document)
