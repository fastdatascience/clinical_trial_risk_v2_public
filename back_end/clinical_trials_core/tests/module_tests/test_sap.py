...
class TestSapExtractor(unittest.TestCase):
    ...
    def test_sap_present(self):
        ...
        for i in range(20):
            pages.append(
                Page(
                    content="center transmission normal content nothing special here",
                    page_number=i,
                )
            )
        for i in range(20):
            pages.append(
                Page(
                    content="pe sap tabulated hazard inferiority statistics maths sample size significance level blah",
                    page_number=i,
                )
            )
        for i in range(20):
