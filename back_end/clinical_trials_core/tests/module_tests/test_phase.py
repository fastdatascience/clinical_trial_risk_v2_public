...
class TestPhaseExtractor(unittest.TestCase):
    ...
    def test_simple_phase_i_present(self):
        document = Document(
            pages=[Page(content="this is a phase i trial", page_number=1)]
        )
    ...
    def test_simple_phase_i_ii_present(self):
        document = Document(
            pages=[Page(content="this is a phase i ii trial", page_number=1)]
        )
        d_result = d.process(document=document)
    ...
    def test_viral_decay(self):
        document = Document(
            pages=[Page(content="this is a phase i viral decay", page_number=1)]
        )
        d_result = d.process(document=document)
