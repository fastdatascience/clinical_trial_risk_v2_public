...
class TestCountryExtractor(unittest.TestCase):
    ...
    def test_gb(self):
        document = Document(
            pages=[Page(content="this is a hiv trial in the UK", page_number=1)]
        )
    ...
    def test_us(self):
        document = Document(
            pages=[Page(content="this is a hiv trial in the USA", page_number=1)]
        )
        d_result = d.process(document=document)
    ...
    def test_annotations_correct_char_indices_demonym(self):
        document = Document(
            pages=[Page(content="We recruited 100 Zimbabwean women", page_number=1)]
        )
        d_result = d.process(document=document)
