...
class TestOvernightStay(unittest.TestCase):
    ...
    def test_no(self):
        document = Document(
            pages=[
                Page(
                    content="There are no overnight stays in the clinic.", page_number=1
                )
            ]
        )
    ...
    def test_yes(self):
        document = Document(
            pages=[Page(content="Patients with planned overnight stay", page_number=1)]
        )
        d_result = d.process(document=document)
    ...
    def test_number(self):
        document = Document(
            pages=[
                Page(
                    content="$150.00 for each overnight stay (10 total)", page_number=1
                )
            ]
        )
        d_result = d.process(document=document)
