...
class TestSampleSizeExtractor(unittest.TestCase):
    ...
    def test_simple_num_subjects_present(self):
        document = Document(pages=[Page(content="we will recruit 1000 subjects", page_number=1)])
        d_result = d.process(document=document)
        self.assertEqual(1000, d_result["prediction"])

    def test_2500(self):
        document = Document(pages=[Page(content="Target enrollment is 2500 participants", page_number=1)])
        d_result = d.process(document=document)
        self.assertEqual(2500, d_result["prediction"])

    def test_exclude_contents_page(self):
        document = Document(pages=[Page(content="Study population 31 6.1 subject inclusion criteria", page_number=1)])
        d_result = d.process(document=document)
        self.assertEqual(0, d_result["prediction"])
