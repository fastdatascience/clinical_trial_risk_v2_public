...
telephone_words= ["tel","call","phone","telephone","fax","efax","facsimile","office hours","out of hours","helpline","mobile","switchboard","direct","number","call","no"]


patterns = {}
for prefix, country_code in dialling_codes:
    if country_code not in patterns:
        patterns[country_code] = []

    for word in telephone_words:
        patterns[country_code].append(f"{word} {prefix}")
        patterns[country_code].append(f"{word}: {prefix}")
...
#     r"(?i)(?:tel|call|phone|telephone|fax|efax|facsimile|office hours|out of hours|helpline|mobile|switchboard)(?: (?:direct|number|call|no))?(?:\W+\w+)?\W+\+\d{1,3}[ -]?\d+[- ]?\d*[- ]?\d*\b")
...
def find_phone_numbers(doc) -> list:
    """
