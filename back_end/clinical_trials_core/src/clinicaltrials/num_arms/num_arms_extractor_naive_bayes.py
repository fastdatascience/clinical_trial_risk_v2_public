    ...
    def process(self, tokenised_pages: list) -> tuple:
        ...
        for page_no, tokens in enumerate(tokenised_pages):
            ...
            for token_idx, token in enumerate(tokens):
                ...
                if token_lower in self.vectoriser.vocabulary_ and not is_number_regex.match(token.text):
                    token_counts[0, self.vectoriser.vocabulary_[token_lower]] += 1

                if token_idx < len(tokens) - 1:
                    tokens_lower = (token.text + " " + tokens[token_idx + 1].text).lower()
        ...
        for page_no, tokens in enumerate(tokenised_pages):
            for token in tokens:
                if token.text.lower() in arms_to_pages:
                    arms_to_pages[token.text.lower()].append(page_no)
