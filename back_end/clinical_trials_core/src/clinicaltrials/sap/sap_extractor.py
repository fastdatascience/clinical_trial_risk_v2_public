...
def derive_feature(f):
    return [max(f), len([x for x in f if x > 0.5]), sum([x for x in f if x > 0.5]), sum(f), len(f), sum(f[-10:]), sum(f[-20:])]


class SapExtractor:
    def __init__(self, path_to_classifier):
        if not exists(path_to_classifier):
            print(f"WARNING! UNABLE TO LOAD SAP CLASSIFIER {path_to_classifier}. You need to run the training script.")
            self.model = None
            return
    ...
    def process(self, tokenised_pages: list) -> tuple:
        ...
        for tokens in tokenised_pages:
            ...
            page_to_probas.append(float(prediction_probas[1]))
        ...
        for page_no, tokens in enumerate(tokenised_pages):
            for token in tokens:
                if token.text.lower() in sap_to_pages:
                    sap_to_pages[token.text.lower()].append(page_no)
        ...
        return {"prediction": prediction_idx, "pages": sap_to_pages, "page_scores": page_to_probas, "score": float(probability_of_sap[1])}
