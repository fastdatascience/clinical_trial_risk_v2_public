...
phase_synonyms = {
    0: ["phase 0"],  # , 'pilot study'],
    0.5: [  # 'early phase i', 'early phase 1', 'early phase 1.0',
        "phase 0.5"
    ],
    1: ["phase i", "phase 1 b", "phase 1", "phase 1.0", "phase ib", "phase ia"],
    1.5: ["phase i ii", "phase 1 2", "phase 1 2 a", "phase 1 - 2", "phase 1-2", "phase i iia", "phase 1 2a",
          "phase 1/2a", "phase ib/ii", "phase ib ii", "phase ib / ii"],
    2: ["phase ii", "phase 2 b", "phase 2", "phase 2.0", "phase iib", "phase iia", "phase 2 a", "phase 2a"],
    2.5: ["phase 2.5", "phase ii iii", "phase 2 3", "phase 2 - 3", "phase 2-3", "phase 2 3a", "phase 2 3b",
          "phase 2/3a", "phase 2/3b", "phase 2 - 3a", "phase 2 - 3b"],
    3: ["phase iii", "phase 3", "phase 3.0", "phase 3b", "phase 3a", "phase iiia", "phase iiib"],
    4: ["phase iv", "phase 4", "phase 4.0", "phase 4 a", "phase 4 b", "phase 4a", "phase 4b", "phase iva", "phase ivb"],
}
...
patterns["this X phase"] = [
    "this proposed phase",
    "this planned phase",
    "this prospective phase",
    "this exploratory phase",
    "this ongoing phase",
    "this randomized phase",
    "this randomised phase",
    "this multicenter phase",
    "this multi-center phase",
    "this multi center phase",
    "this multicentre phase",
    "this multi-centre phase",
    "this multi centre phase",
    "this pivotal phase",
    "this combined phase",
    "this single phase",
...
context_matcher = PhraseMatcher(nlp.vocab, attr="LOWER")
...
def extract_features(spacy_docs):
    # orig_tokenized_pages stores the original case information
    phase_to_pages = {}
    ...
    for page_number, doc in enumerate(spacy_docs):
        ...
        for word, start, end in sorted(phrase_matches, key=lambda t: t[2] - t[1], reverse=True):
            ...
            context_end = min(len(doc) - 1, end + 15)
            context = doc[context_start : context_end].text
            contexts[phase_number] = (contexts[phase_number] + " " + f"Page {page_number + 1}: " + context).strip()

            match_start_char = doc[start].idx
            match_end_char = doc[end].idx+len(doc[end].text)
            match_text = doc[start:end].text
            annotations.append(
                {"type": "phase", "subtype": phase_number, "page_no": page_number, "start_char": match_start_char,
    ...
    for idx, (phase, pages) in enumerate(phase_to_pages):
        ...
        feat_vect = [
            ...
            max(pages),
            idx,
            (max(pages) - min(pages)) / len(spacy_docs),
            phase_to_title_case.get(phase, 0),
            phase_to_upper_case.get(phase, 0),
        ]
    ...
    return X, phase_to_pages, contexts, annotations
...
class PhaseExtractorRuleBased:
    ...
    def process(self, spacy_docs: list) -> tuple:
        """
        ...
        X, phase_to_pages, contexts, annotations = extract_features(spacy_docs)
        ...
        return {"prediction": prediction, "pages": dict(phase_to_pages), "probas": scores, "context": contexts, "annotations":annotations}
